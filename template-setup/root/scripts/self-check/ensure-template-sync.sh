...
# This script reads TEMPLATE_REPO_URL from .env file and syncs the entire template
...
# Validate URL
[[ "$TEMPLATE_REPO_URL" == *.zip ]] || { echo "Error: TEMPLATE_REPO_URL must end with .zip"; exit 1; }

# Download and extract template
echo "Downloading and extracting template..."
wget --secure-protocol=auto -O "$TEMP_DIR/template.zip" "$TEMPLATE_REPO_URL"
